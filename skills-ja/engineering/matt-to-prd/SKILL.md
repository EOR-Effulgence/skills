...
2. 実装に必要な構築・改修対象の主要モジュールをスケッチする。**深いモジュール（deep modules）**を切り出し、単独でテストできる機会を能動的に探す。

   深いモジュールとは（浅いモジュールの対義語）、多くの機能を、めったに変わらないシンプルかつテスト可能なインターフェースの裏に隠したモジュールのこと。

   モジュール群がユーザーの期待と一致するか確認する。どのモジュールにテストを書きたいか確認する。
