...
ローカルなテスト代替物を持つ依存（Postgres なら PGLite、ファイルシステムなら in-memory 実装）。代替物があれば deep 化可能。deep 化 Module はテストスイート内で代替物を走らせてテストする。seam は内部にあり、Module の外部 interface には port を出さない。
