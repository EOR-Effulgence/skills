...
Agent ツールで 3 つ以上の sub-agent を並列に生やす。各 agent は deep 化モジュールについて**根本的に異なる** interface を作らねばならない。
...
設計を順に提示し、ユーザーが 1 つずつ吸収できるようにしてから、散文で比較する。**depth**（interface でのレバレッジ）、**locality**（変更がどこに集まるか）、**seam の位置**で対比する。
